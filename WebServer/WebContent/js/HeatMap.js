...
var count = 0;
var pointsArray = pointArray = new google.maps.MVCArray([]);
var locations = {};
...
function sendMsg() {
	...
	$.get(server, data, function(resp) {
		...
		if (resp == "no matching record") {
			document.getElementById("status").innerHTML = "No New Points";
		} else {
			update(resp);
		}
	}).fail(function() {
...
function update(resp) {
	...
	for (var i = 0; i < points.length; i++) {
		...
		if (point.id in locations) {
			continue;
		}
		locations[point.id] = new google.maps.LatLng(point.lat, point.lng)
		pointArray.push(locations[point.id]);
	}
	document.getElementById("status").innerHTML = points.length;
}
